function main() {
    ...
    class Subject {
        ...
        getUrlExtraction() {
            if (stageLiveDecide) {
                ...
                if(applianceFinder.selectedParameters.includes("WM_Type")){
                    resultPageUrl = 'https://publish-p86205-e734304.adobeaemcloud.com/uk/laundry/washing-machines/';
                } else {
                    resultPageUrl = 'https://publish-p86205-e734304.adobeaemcloud.com/uk/laundry/washer-dryers/';
                }
            }
            let parSave = "";
            for (let [key, value] of Object.entries(applianceFinder.resultParamSet)) {
    ...
    $nextBtn.on('click', function () {
        ...
        if (idx !== 0) {
            if (idx === 1) {
                if(applianceFinder.selectedParameters.includes("WM_Type")){
                    applianceFinder.productSpec = WMProductSpec;
                } else {
                    applianceFinder.productSpec = WDProductSpec;
                }
                for (let [key, value] of Object.entries(applianceFinder.productSpec)) {
                    newProduct.push(key);
                }
            } else {
        ...
        if (idx === 0) {
            ...
            for (let [key, value] of Object.entries(applianceFinder.selectProduct[idx - 1])) {
                concatArr += Object.values(applianceFinder.productSpec[value]) + ",";
            }
    ...
    $showNow.on('click', function () {
        if ($finderMain.hasClass('ready')) {
            ...
            if (idx === 0) {
                //applianceFinder.productSpec
                if($(".option_btn.active").data("value") === "WM_Type"){
                    applianceFinder.productSpec = WMProductSpec;
                    for (let [key, value] of Object.entries(applianceFinder.productSpec)) {
                        newProduct.push(key);
                    }
            ...
            // 확인용
            if(applianceFinder.selectedParameters.includes("WM_Type")){
                applianceFinder.resultParamSet = WMResultParamSet;
            } else {
                applianceFinder.resultParamSet = WDResultParamSet;
            }
